...
public class LightWave extends ActiveSkill {
	...
	public SkillResult use(Player player) {
		...
		blocks[2] = player.getLocation().getBlock().getRelative(faces[DUtils.FRONT]).getRelative(BlockFace.UP);
		blocks[1] = blocks[2].getRelative(faces[DUtils.LEFT]).getRelative(faces[DUtils.BACK]);
		blocks[0] = blocks[1].getRelative(faces[DUtils.LEFT]).getRelative(faces[DUtils.BACK]);
		blocks[3] = blocks[1].getRelative(faces[DUtils.RIGHT], 2);
		blocks[4] = blocks[0].getRelative(faces[DUtils.RIGHT], 4);
		new LightWaveEffect(plugin, player, blocks, faces[DUtils.FRONT], distance).start();
	...
	public class LightWaveEffect extends TickEffect {
		...
		public LightWaveEffect(DragonSkillsPlugin plugin, Player player, Block[] blocks, BlockFace front, int distance) {
		...
		public void restore() {
			for (int i = 0; i < states.length; i++) {
				if (states[i] != null && states[i].getBlock().getType() == Material.GOLD_BLOCK) {
